...
type
  TGitRecentCommentsDialog = class(TForm)
    Panel1: TPanel;
    Splitter1: TSplitter;
    Panel2: TPanel;
    Panel3: TPanel;
...
function SelectRecentComments(const AOwner: TComponent; const Comments: TStringList): string;
var
  RecentCommentsDialog: TGitRecentCommentsDialog;
begin
  RecentCommentsDialog := TGitRecentCommentsDialog.Create(AOwner);
...
procedure TGitRecentCommentsDialog.FormCreate(Sender: TObject);
begin
...
procedure TGitRecentCommentsDialog.RecentCommentClick(Sender: TObject);
begin
...
procedure TGitRecentCommentsDialog.RecentCommentDblClick(Sender: TObject);
begin
  if RecentComment.ItemIndex <> -1 then
  begin
    Comment.Text := RecentComment.Items[RecentComment.ItemIndex];
