...
function GetUpdateDialog(const Url: string; AAbortCallBack: TAbortCallBack;
  ...
  AFileRefreshCallBack: TFileRefreshCallBack): TGitUpdateDialog;
...
function GetUpdateDialog(const Url: string; AAbortCallBack: TAbortCallBack;
  AResolveCallBack: TResolveCallBack; AFileRefreshCallBack: TFileRefreshCallBack): TGitUpdateDialog;
begin
  Result := TGitUpdateDialog.Create(Application);
...
procedure TGitUpdateDialog.AbortClick(Sender: TObject);
begin
  FAbortCallBack;
end;

procedure TGitUpdateDialog.Add(const FileName, Action: string;
...
procedure TGitUpdateDialog.Completed;
var
...
procedure TGitUpdateDialog.FilesCustomDrawItem(Sender: TCustomListView;
...
procedure TGitUpdateDialog.FormCreate(Sender: TObject);
var
  I: Integer;
begin
...
procedure TGitUpdateDialog.HelpClick(Sender: TObject);
begin
  Application.HelpContext(HelpContext);
end;

procedure TGitUpdateDialog.OkClick(Sender: TObject);
{
var
  I: Integer;
}
...
procedure TGitUpdateDialog.ResolveActionExecute(Sender: TObject);
begin
  {
...
procedure TGitUpdateDialog.ResolveActionUpdate(Sender: TObject);
begin
  if Assigned(Files.Selected) then
